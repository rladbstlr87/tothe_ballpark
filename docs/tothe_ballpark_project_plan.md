...
*   **핵심 목표:**
    ...
    4.  **CI/CD 파이프라인:** `Docker`와 `GitHub Actions`를 활용한 빌드/테스트/배포 자동화 구축
...
#### **Phase 1: 기반 다지기 (Foundation & Containerization)**

*   **목표:** `PostgreSQL` 도입 및 `Docker`를 통한 개발 환경 표준화.
*   **세부 계획:**
    ...
    3.  **Dockerize:** `Dockerfile` 및 `docker-compose.yml` 작성하여 Django, PostgreSQL 등을 컨테이너 환경에서 실행.
        *   **이유:** 컨테이너 기술을 통해 개발 환경과 프로덕션 환경의 일관성을 보장하고, "Infrastructure as Code" 원칙을 적용합니다.
            *   **환경 차이로 인한 오류 제거:** 모든 개발자와 서버는 동일한 환경 위에서 애플리케이션을 실행하게 되므로, "내 컴퓨터에선 되는데..."와 같은 고질적인 문제를 원천적으로 차단합니다.
            *   **개발 환경 구축 자동화:** `docker-compose.yml` 파일 하나로 프로젝트에 필요한 모든 서비스(웹, DB, 캐시 등)를 명령어 한 줄로 실행할 수 있어, 신규 참여자의 온보딩 과정을 획기적으로 단축시킵니다.
            *   **MSA 전환의 초석:** 각 서비스를 독립된 컨테이너로 관리하는 경험은 향후 마이크로서비스 아키텍처로의 전환을 위한 기술적 기반이 됩니다.
...
*   **세부 계획:**
    1.  **CI (Continuous Integration):**
        ...
        *   테스트 통과 시 `Dockerfile`을 빌드하여 Docker Hub 또는 GitHub Container Registry에 이미지 Push.
    2.  **CD (Continuous Deployment):**
        *   (심화) 서버에서 최신 Docker 이미지를 Pull 받아 자동으로 재배포하는 스크립트 실행.
...
| **Infra** | Docker, Docker Compose | 애플리케이션을 격리된 컨테이너 환경에서 실행하여 개발 및 배포 환경을 표준화. |
...
*   **효율적인 개발 및 운영 문화 구축:** Docker를 통한 개발 환경 표준화와 CI/CD 파이프라인을 통한 배포 자동화는 개발 생산성을 향상시키고, 안정적인 서비스 운영을 가능하게 합니다.